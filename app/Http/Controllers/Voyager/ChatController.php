...
class ChatController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        //Update new messages count in chat_room table
        $room_ids = ChatRoom::get('id');
        foreach ($room_ids as $room){
            $count = Messages::where([['user_id','!=', auth()->user()->id],['chat_room_id','=',$room->id], ['status','=', 'NEW']])->count();
            ChatRoom::where('id', $room->id)->update(['user_new_count'=>$count]);
        }
